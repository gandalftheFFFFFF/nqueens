...
public class JOHNFUCKINGMADDEN {
	private int x = 0;
	private int y = 0;
	private int[][] board;
	// my vars:
	...
	public void initializeGame(int size) {
		...
		buildBDD();
		// Run update invalid (Not really needed in this case, but good
		// practice)
		updateInvalid();
	}
	...
	private void createCellRule(int x, int y) {
		...
		for (int yy = 0; yy < N; yy++) {
			if (y != yy) {
				rest_false_bdd = rest_false_bdd.and(this.factory.nithVar(place(
						x, yy)));
			}
		}
		...
		for (int xx = 0; xx < N; xx++) {
			if (x != xx) {
				if ((y + xx - x < 8) && (y + xx - x > 0)) {
					rest_false_bdd = rest_false_bdd.and(this.factory
							.nithVar(place(xx, y + xx - x)));
				}
			}
		}
	...
	private boolean placeInvalid(int column, int row) {
		// add queen at x ,y
		BDD test_bdd = this.bdd.restrict(this.factory
				.ithVar(place(column, row)));
	...
	public boolean insertQueen(int column, int row) {
		...
		this.bdd = this.bdd
				.restrict(this.factory.ithVar(row * this.N + column));
		updateInvalid();
		return true;
	}
}